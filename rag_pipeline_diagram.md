...
```mermaid
flowchart TD
    A[Markdown documents in script/data/documents] --> B[Chunk documents]
    B --> C[Generate chunk embeddings with all-MiniLM-L6-v2]
    C --> D[Store vectors in FAISS IndexFlatIP]
    D --> E[Vector store files in script/data/vector_store]
...
1. Documents are stored as Markdown files in `script/data/documents`.
2. `rag_pipeline.py prepare` loads the documents, chunks them, generates embeddings, and writes the FAISS index to `script/data/vector_store`.
...
|---|---|
| `script/data/documents/` | Input Markdown documents |
| `script/data/vector_store/` | Saved FAISS index, chunk metadata, and manifest |
| `script/data/rag_pipeline/rag_pipeline.py` | RAG implementation |
| `script/data/rag_pipeline/evaluate.py` | 10-question evaluation script |
| `script/data/results/` | Evaluation outputs |
